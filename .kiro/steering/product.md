...
- **デバッグ・開発者機能**:
  - **統合デバッグダッシュボード**: 開発・テスト・トラブルシューティング用の包括的なデバッグUI
  - **テスト通知送信**: カスタムメッセージによる通知即時配信テスト機能
  - **スケジュール通知一覧**: 予約済み通知の確認・詳細表示・個別キャンセル機能
  - **カレンダーデータ可視化**: 取得済みGoogleカレンダーイベントのリアルタイム表示・検証
  - **隙間時間検出表示**: 検出された隙間時間・空き時間の詳細情報と条件確認
  - **API通信ログ**: Google Calendar API通信履歴・レスポンス・エラーログの記録と閲覧
  - **設定統合**: SettingsView内の専用デバッグセクションからアクセス可能
  - **デバッグモード管理**: UserDefaultsベースのデバッグモードON/OFF切り替え
