...
# ★★★ 音声ファイルのマッピング ★★★
AUDIO_FILE_MAP = {
    'Elbows Out Error': "audio/elbow_error.wav",
    'Hips Sag Error': "audio/hip_sag.wav",
...
def process_keypoints_for_inference(frame_keypoints_xy):
    ...
    kps = {'nose': 0, 'left_shoulder': 5, 'right_shoulder': 6, 'left_elbow': 7, 'right_elbow': 8, 'left_wrist': 9, 'right_wrist': 10, 'left_hip': 11, 'right_hip': 12}
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if results[0].keypoints and results[0].keypoints.shape[1] > 0:
            ...
            kps_to_check = [keypoints_xy[i] for i in [5, 6, 7, 8, 9, 10, 11, 12]]
        ...
        if body_is_visible:
            ...
            current_angle = (calculate_angle(l_sh, l_el, l_wr) + calculate_angle(r_sh, r_el, r_wr)) / 2
            ...
            if state == 'up' and smoothed_angle < DOWN_ANGLE_THRESHOLD:
                state = 'down'; rep_keypoints_sequence.clear()
            elif state == 'down' and smoothed_angle > UP_ANGLE_THRESHOLD:
                state = 'up'; rep_counter += 1
            ...
            state = 'up'; rep_keypoints_sequence.clear(); angle_history.clear()
            # ★★★ 修正点: 運動開始前(0回)の場合のみ"Ready"に戻す ★★★
            if rep_counter == 0:
                last_verdict, verdict_color = "Ready", (255, 255, 255)
